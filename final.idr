...
--the Exp data type is used to represent expressions in the language. Expressions are syntactic constructs that represent a computation or task to be performed. 
--So overall, the Exp data type is used to represent the syntax and semantics of expressions in the language.
...
evalProg (MkProgram p_funDecl p_return_type (ExpIfThenElse x y z)) = if evalProg ( MkProgram p_funDecl Tbool x) then evalProg ( MkProgram p_funDecl p_return_type y) else evalProg (MkProgram p_funDecl p_return_type z)
